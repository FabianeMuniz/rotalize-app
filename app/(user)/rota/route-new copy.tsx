...
export default function NewRouteScreen() {
  ...
  const removeStop = (id: string) => {
    ...
    setInitialIndex((prev) => Math.max(0, Math.min(prev, Math.max(0, stops.length - 2))));
  };
  ...
  const onGenerate = async () => {
    if (!canGenerate) {
      Alert.alert("Atenção", "Informe um nome para a rota e selecione ao menos dois pontos.");
      return;
    }
    try {
      setSubmitting(true);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.panel}>
          ...
          <Text style={styles.subtitle}>Selecione os endereços e marque o ponto inicial</Text>
          ...
          <ScrollView
            ...
            contentContainerStyle={{ paddingBottom: insets.bottom + 140 + TABBAR_H }}
          >
            {/* Nome da rota */}
          ...
          <View style={[styles.footer,
            ...
            <Pressable
              ...
              android_ripple={{ color: "rgba(255,255,255,0.15)" }}
              style={[styles.primaryBtn, { marginTop: 0 }, (!canGenerate || submitting) && { opacity: 0.7 }]}
              accessibilityRole="button"
...
function StopInput({
  ...
  // busca no Nominatim com debounce
  useEffect(() => {
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      <View style={styles.inputWrap}>
        ...
        />
        {/* + abaixo */}
        <Pressable onPress={onAddAfter} hitSlop={10} accessibilityLabel="Adicionar novo ponto">
          <FontAwesome5 name="plus" size={14} color={COLORS.navy} />
        </Pressable>
        <View style={{ width: 8 }} />
        {/* remover */}
        <Pressable onPress={onRemove} hitSlop={10} accessibilityLabel="Remover ponto">
          ...
          <ScrollView nestedScrollEnabled style={{ maxHeight: 220 }}>
            {results.map((r: NominatimItem, i: number) => (
              <Pressable
                ...
                style={({ pressed }) => [styles.suggestionItem, pressed && { opacity: 0.85 }]}
              >
                <Text numberOfLines={2} style={styles.suggestionText}>
                  {r.display_name}
                </Text>
                <Text style={styles.suggestionSub}>
                  ({formatLatLon(parseFloat(r.lat))}, {formatLatLon(parseFloat(r.lon))})
                </Text>
              </Pressable>
            ))}
          </ScrollView>
